...
export default function RankIcon({ tier, rank, size = 'md' }: RankIconProps) {
  ...
  return (
    ...
    >
      ...
      {/* Tier text */}
      <div className={`font-bold text-white drop-shadow-lg ${size === 'lg' ? 'text-lg' : size === 'md' ? 'text-sm' : 'text-xs'}`}>
        {tier.slice(0, 3)}
      </div>
      
      {/* Rank text */}
      <div className={`font-semibold text-white/90 ${size === 'lg' ? 'text-sm' : 'text-xs'}`}>
        {rank}
      </div>
    </div>
  );
